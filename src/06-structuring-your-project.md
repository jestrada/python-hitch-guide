...
*For advice on writing your tests, see* `/writing/tests`{.interpreted-text role="doc"}.
...
- Multiple and messy circular dependencies: If the classes Table and Chair in `furn.py`{.interpreted-text role="file"} need to import Carpenter from `workers.py`{.interpreted-text role="file"} to answer a question such as `table.isdoneby()`, and if conversely the class Carpenter needs to import Table and Chair to answer the question `carpenter.whatdo()`, then you have a circular dependency. In this case you will have to resort to fragile hacks such as using import statements inside your methods or functions.
...
As soon as you use [import]{.title-ref} statements, you use modules. These can be either built-in modules such as [os]{.title-ref} and [sys]{.title-ref}, third-party modules you have installed in your environment, or your project"s internal modules.

To keep in line with the style guide, keep module names short, lowercase, and be sure to avoid using special symbols like the dot (.) or question mark (?). A file name like `my.spam.py`{.interpreted-text role="file"} is the one you should avoid! Naming this way will interfere with the way Python looks for modules.

In the case of [my.spam.py]{.title-ref} Python expects to find a `spam.py`{.interpreted-text role="file"} file in a folder named `my`{.interpreted-text role="file"} which is not the case. There is an [example](http://docs.python.org/tutorial/modules.html#packages) of how the dot notation should be used in the Python docs.

If you like, you could name your module `my_spam.py`{.interpreted-text role="file"}, but even our trusty friend the underscore, should not be seen that often in module names. However, using other characters (spaces or hyphens) in module names will prevent importing (- is the subtract operator). Try to keep module names short so there is no need to separate words. And, most of all, don"t namespace with underscores; use submodules instead.
...
Concretely, the `import modu` statement will look for the proper file, which is `modu.py`{.interpreted-text role="file"} in the same directory as the caller, if it exists. If it is not found, the Python interpreter will search for `modu.py`{.interpreted-text role="file"} in the "path" recursively and raise an ImportError exception when it is not found.

When `modu.py`{.interpreted-text role="file"} is found, the Python interpreter will execute the module in an isolated scope. Any top-level statement in `modu.py`{.interpreted-text role="file"} will be executed, including other imports if any. Function and class definitions are stored in the module"s dictionary.
...
Any directory with an `__init__.py`{.interpreted-text role="file"} file is considered a Python package. The different modules in the package are imported in a similar manner as plain modules, but with a special behavior for the `__init__.py`{.interpreted-text role="file"} file, which is used to gather all package-wide definitions.

A file `modu.py`{.interpreted-text role="file"} in the directory `pack/`{.interpreted-text role="file"} is imported with the statement `import pack.modu`. This statement will look for `__init__.py`{.interpreted-text role="file"} file in `pack`{.interpreted-text role="file"} and execute all of its top-level statements. Then it will look for a file named `pack/modu.py`{.interpreted-text role="file"} and execute all of its top-level statements. After these operations, any variable, function, or class defined in `modu.py`{.interpreted-text role="file"} is available in the pack.modu namespace.

A commonly seen issue is adding too much code to `__init__.py`{.interpreted-text role="file"} files. When the project complexity grows, there may be sub-packages and sub-sub-packages in a deep directory structure. In this case, importing a single item from a sub-sub-package will require executing all `__init__.py`{.interpreted-text role="file"} files met while traversing the tree.

Leaving an `__init__.py`{.interpreted-text role="file"} file empty is considered normal and even good practice, if the package"s modules and sub-packages do not need to share any code.

Lastly, a convenient syntax is available for importing deeply nested packages: `import very.deep.module as mod`. This allows you to use [mod]{.title-ref} in place of the verbose repetition of `very.deep.module`.
...
Some coding practices, like functional programming, recommend never reassigning a variable. In Java this is done with the [final]{.title-ref} keyword. Python does not have a [final]{.title-ref} keyword and it would be against its philosophy anyway. However, it may be a good discipline to avoid assigning to a variable more than once, and it helps in grasping the concept of mutable and immutable types.

## Mutable and immutable types

Python has two kinds of built-in or user-defined types.

Mutable types are those that allow in-place modification of the content. Typical mutables are lists and dictionaries: All lists have mutating methods, like `list.append`{.interpreted-text role="py:meth"} or `list.pop`{.interpreted-text role="py:meth"}, and can be modified in place. The same goes for dictionaries.
...
You can also use the `% <python:string-formatting>`{.interpreted-text role="ref"} formatting operator to concatenate a pre-determined number of strings besides `str.join`{.interpreted-text role="py:meth"} and `+`. However, `3101`{.interpreted-text role="pep"} discourages the usage of the `%` operator in favor of the `str.format`{.interpreted-text role="py:meth"} method.
:
