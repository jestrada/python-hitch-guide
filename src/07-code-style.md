...
In Python 3, a new method of extended unpacking was introduced by `3132`{.interpreted-text role="pep"}:
...
If you need to assign something (for instance, in `unpacking-ref`{.interpreted-text role="ref"}) but will not need that variable, use `__`:
...
Because lists are mutable, the `*` operator (as above) will create a list of N references to the [same]{.title-ref} list, which is not likely what you want. Instead, use a list comprehension:
...
A common idiom for creating strings is to use `str.join`{.interpreted-text role="py:meth"} on an empty string.
...
Also known as `20`{.interpreted-text role="pep"}, the guiding principles for Python"s design.
...
`8`{.interpreted-text role="pep"} is the de facto code style guide for Python. A high quality, easy-to-read version of PEP 8 is also available at [pep8.org](http://pep8.org/).
...
While autopep8 focuses on solving the PEP 8 violations, [yapf](https://github.com/google/yapf) tries to improve the format of your code aside from complying with PEP 8. This formatter aims at providing as good looking code as a programmer who writes PEP 8 compliant code. It gets installed with:
...
Don"t use the `dict.has_key`{.interpreted-text role="py:meth"} method. Instead, use `x in d` syntax, or pass a default argument to `dict.get`{.interpreted-text role="py:meth"}.
...
[Generator expressions](http://docs.python.org/tutorial/classes.html#generator-expressions) follows almost the same syntax as list comprehensions but return a generator instead of a list.
...
Use `enumerate`{.interpreted-text role="py:func"} keep a count of your place in the list.
...
The `enumerate`{.interpreted-text role="py:func"} function has better readability than handling a counter manually. Moreover, it is better optimized for iterators.
